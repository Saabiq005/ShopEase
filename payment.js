...
async function fetchCustomerCreditScore() {
    try {
        const response = await fetch(`${baseFirestoreUrl}/Customer/${customerId}?key=${apiKey}`);
        const customerData = await response.json();
        customerCreditScore = customerData.fields?.Credit?.integerValue || 0;
...
import { performPaymentLogic } from './performpayment.js';
    ...
    try {
        ...
        const orderData = {
            fields: {
                ...
                cash: { integerValue: cashAmount },
                credit: { integerValue: creditAmount },
                products: {
                    arrayValue: {
                            ...
                            return {
                                mapValue: {
                                    fields: {
                                        ...
                                        totalProductPrice: { integerValue: totalProductPrice },
